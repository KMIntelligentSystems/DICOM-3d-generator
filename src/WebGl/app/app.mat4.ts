/**
     ...
     */
    //var mat3 = {};
    let MatrixArray = (typeof Float32Array !== 'undefined') ? Float32Array : Array;

    let FLOAT_EPSILON = 0.000001;
         ...
         */
       export function createmat3(mat) {
         ...
         * @returns {mat3} New mat3
         */
        export function createMat3From (m00, m01, m02, m10, m11, m12, m20, m21, m22) {
            var dest = new MatrixArray(9);
         ...
         */
        export function determinantMat3(mat) {
         ...
         */
        export function inverseMat3(mat, dest) {
            var a00 = mat[0], a01 = mat[1], a02 = mat[2],
                a10 = mat[3], a11 = mat[4], a12 = mat[5],
                a20 = mat[6], a21 = mat[7], a22 = mat[8],
         ...
         */
        export function multiplyMat3(mat, mat2, dest) {
            if (!dest) { dest = mat; }
         ...
         **/
        export function multiplyVec2Mat3(matrix, vec, dest) {
          if (!dest) dest = vec;
          var x = vec[0], y = vec[1];
         ...
         **/
        export function multiplyVec3Mat3(matrix, vec, dest) {
          if (!dest) dest = vec;
         ...
         * @returns {mat3} dest
         */
        export function setMat3(mat, dest) {
            dest[0] = mat[0];
            dest[1] = mat[1];
            dest[2] = mat[2];
            dest[3] = mat[3];
         ...
         */
        export function transposeMat3(mat, dest) {
         ...
         */
        export function mat3toMat4(mat, dest) {
            if (!dest) { dest = createMat4(null); }
         ...
         */
        export function createMat4(mat) {
         ...
         * @returns {mat4} New mat4
         */
        export function createFromMat4(m00, m01, m02, m03, m10, m11, m12, m13, m20, m21, m22, m23, m30, m31, m32, m33) {
            var dest = new MatrixArray(16);
         ...
         * @returns {mat4} dest
         */
        export function setMat4(mat, dest) {
            dest[0] = mat[0];
            dest[1] = mat[1];
            dest[2] = mat[2];
            dest[3] = mat[3];
         ...
         */
        export function toRotationMat(mat, dest) {
            if (!dest) { dest = createMat4(null); }
         ...
         */
        export function mat4toMat3(mat, dest) {
            if (!dest) { dest = createmat3(null); }
         ...
         */
        export function mat4toInverseMat3(mat, dest) {
            // Cache the matrix values (makes for huge speed increases!)
            var a00 = mat[0], a01 = mat[1], a02 = mat[2],
         ...
         */
        export function multiplyMat4(mat, mat2, dest) {
            if (!dest) { dest = mat; }
         ...
         */
        export function mat4multiplyVec3(mat, vec, dest) {
            if (!dest) { dest = vec; }
         ...
         */
        export function mat4multiplyVec4(mat, vec, dest) {
            if (!dest) { dest = vec; }
         ...
         * @returns {mat4} dest if specified, mat otherwise
         */
        export function translateMat4(mat, vec, dest) {
            var x = vec[0], y = vec[1], z = vec[2],
                a00, a01, a02, a03,
                a10, a11, a12, a13,
                a20, a21, a22, a23;
         ...
         */
        export function scaleMat4(mat, vec, dest) {
            var x = vec[0], y = vec[1], z = vec[2];
         ...
         * @returns {mat4} dest if specified, mat otherwise
         */
        export function rotateMat4(mat, angle, axis, dest) {
         ...
         * @returns {mat4} dest if specified, mat otherwise
         */
        export function rotateXMat4(mat, angle, dest) {
         ...
         * @returns {mat4} dest if specified, mat otherwise
         */
        export function rotateYMat4(mat, angle, dest) {
            var s = Math.sin(angle),
                c = Math.cos(angle),
                a00 = mat[0],
                a01 = mat[1],
         ...
         * @returns {mat4} dest if specified, mat otherwise
         */
        export function rotateZMat4(mat, angle, dest) {
            var s = Math.sin(angle),
                c = Math.cos(angle),
                a00 = mat[0],
                a01 = mat[1],
         ...
         * @returns {mat4} dest if specified, a new mat4 otherwise
         */
        export function fromRotationTranslation (quat, vec, dest) {
            if (!dest) { dest = createMat4(null); }
         ...
         */
        export function createQuat4(quat) {
         ...
         * @returns {quat4} New quat4
         */
        export function createFromQuat4(x, y, z, w) {
            var dest = new MatrixArray(4);
         ...
         * @returns {quat4} dest
         */
        export function setQuat4(quat, dest) {
            dest[0] = quat[0];
            dest[1] = quat[1];
            dest[2] = quat[2];
            dest[3] = quat[3];
         ...
         * @returns {Boolean} True if a is equivalent to b
         */
        export function equalQuat4(a, b) {
            return a === b || (
                Math.abs(a[0] - b[0]) < FLOAT_EPSILON &&
                Math.abs(a[1] - b[1]) < FLOAT_EPSILON &&
                Math.abs(a[2] - b[2]) < FLOAT_EPSILON &&
         ...
         */
        export function identityQuat4(dest) {
            if (!dest) { dest = createQuat4(null); }
            dest[0] = 0;
            dest[1] = 0;
            dest[2] = 0;
            dest[3] = 1;
        ...
        var identity_Quat4 = identityQuat4(null);
         ...
         */
        export function calculateWQuat4(quat, dest) {
         ...
         */
        export function dotQuat4(quat, quat2){
         ...
         * @returns {quat4} dest if specified, quat otherwise
         */
        export function inverseQuat4(quat, dest) {
         ...
         * @returns {quat4} dest if specified, quat otherwise
         */
        export function conjugateQuat4(quat, dest) {
            if (!dest || quat === dest) {
                quat[0] *= -1;
                quat[1] *= -1;
                quat[2] *= -1;
         ...
         */
        export function lengthQuat4(quat) {
            var x = quat[0], y = quat[1], z = quat[2], w = quat[3];
         ...
         * @returns {quat4} dest if specified, quat otherwise
         */
        export function normalizeQuat4(quat, dest) {
            if (!dest) { dest = quat; }
         ...
         * @returns {quat4} dest if specified, quat otherwise
         */
        export function addQuat4(quat, quat2, dest) {
            if(!dest || quat === dest) {
                quat[0] += quat2[0];
                quat[1] += quat2[1];
                quat[2] += quat2[2];
         ...
         * @returns {quat4} dest if specified, quat otherwise
         */
        export function multiplyQuat4(quat, quat2, dest) {
            if (!dest) { dest = quat; }
         ...
         */
        export function multiplyVec3Quat4(quat, vec, dest) {
            if (!dest) { dest = vec; }
         ...
         * @returns {quat4} dest if specified, quat otherwise
         */
        export function scaleQuat4 (quat, val, dest) {
            if(!dest || quat === dest) {
                quat[0] *= val;
                quat[1] *= val;
                quat[2] *= val;
         ...
         */
        export function quat4toMat3(quat, dest) {
            if (!dest) { dest = createmat3(null); }
         ...
         * @returns {mat4} dest if specified, a new mat4 otherwise
         */
        export function quat4toMat4(quat, dest) {
            if (!dest) { dest = createMat4(null); }
         ...
         * @returns {quat4} dest if specified, quat otherwise
         */
        export function slerpQuat4 (quat, quat2, slerp, dest) {
            if (!dest) { dest = quat; }
         ...
         *
         */
        export function fromRotationMatrixQuat4(mat, dest) {
            if (!dest) dest = createQuat4(null);
            ...
            if ( fTrace > 0.0 ) {
                ...
                // |w| <= 1/2
                var s_iNext = /*fromRotationMatrixQuat4.s_iNext = fromRotationMatrixQuat4.s_iNext ||*/ [1,2,0];
                var i = 0;
                if ( mat[4] > mat[0] )
                  i = 1;
                if ( mat[8] > mat[i*3+i] )
         ...
         */
        let mat3toQuat4 = fromRotationMatrixQuat4;
    
        (function() {
            var mat = createmat3(null);
             ...
             * @returns {quat4} dest
             **/
           /* exportfromAxes4Quat(view, right, up, dest) {
                mat[0] = right[0];
                mat[3] = right[1];
                mat[6] = right[2];
    ...
    */
      ...
      /*  mat2.str = function (mat) {
       ...
       // var vec4 = {};
         ...
         */
        export function createVec4(vec) {
            var dest = new MatrixArray(4);
         ...
         */
        export function createFromVec4(x, y, z, w) {
            var dest = new MatrixArray(4);
         ...
         * @returns {vec4} dest
         */
        export function addVec4(vecA, vecB, dest) {
          if (!dest) dest = vecB;
          dest[0] = vecA[0] + vecB[0];
          dest[1] = vecA[1] + vecB[1];
         ...
         * @returns {vec4} dest
         */
        export function subtractVec4(vecA, vecB, dest) {
          if (!dest) dest = vecB;
          dest[0] = vecA[0] - vecB[0];
          dest[1] = vecA[1] - vecB[1];
          dest[2] = vecA[2] - vecB[2];
         ...
         * @returns {vec4} dest
         */
        export function multiplyVec4(vecA, vecB, dest) {
          if (!dest) dest = vecB;
          dest[0] = vecA[0] * vecB[0];
          dest[1] = vecA[1] * vecB[1];
          dest[2] = vecA[2] * vecB[2];
         ...
         * @returns {vec4} dest
         */
        export function divideVec4(vecA, vecB, dest) {
          if (!dest) dest = vecB;
          dest[0] = vecA[0] / vecB[0];
          dest[1] = vecA[1] / vecB[1];
          dest[2] = vecA[2] / vecB[2];
         ...
         * @returns {vec4} dest
         */
        export function scaleVec4(vecA, scalar, dest) {
          if (!dest) dest = vecA;
          dest[0] = vecA[0] * scalar;
          dest[1] = vecA[1] * scalar;
         ...
         * @returns {vec4} dest
         */
        export function setVec4(vec, dest) {
            dest[0] = vec[0];
            dest[1] = vec[1];
            dest[2] = vec[2];
            dest[3] = vec[3];
         ...
         * @returns {vec4} dest if specified, vec otherwise
         */
        export function negateVec4 (vec, dest) {
            if (!dest) { dest = vec; }
            dest[0] = -vec[0];
            dest[1] = -vec[1];
            dest[2] = -vec[2];
         ...
         * @returns {Number} Length of vec
         */
        export function lengthVec4(vec) {
          var x = vec[0], y = vec[1], z = vec[2], w = vec[3];
          return Math.sqrt(x * x + y * y + z * z + w * w);
        };
         ...
         * @returns {Number} Squared Length of vec
         */
        export function squaredLengthVec4 (vec) {
          var x = vec[0], y = vec[1], z = vec[2], w = vec[3];
         ...
         */
        export function lerpVec4(vecA, vecB, lerp, dest) {
            if (!dest) { dest = vecA; }
            dest[0] = vecA[0] + lerp * (vecB[0] - vecA[0]);
            dest[1] = vecA[1] + lerp * (vecB[1] - vecA[1]);
